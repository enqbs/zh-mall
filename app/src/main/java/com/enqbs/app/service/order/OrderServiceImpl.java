...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderConfirmVO getOrderConfirmVO(OrderConfirmForm form) {
        UserInfoVO userInfoVO = userService.getUserInfoVO();
        String orderTokenKey = String.format(Constants.ORDER_TOKEN_REDIS_KEY, userInfoVO.getUserId());
        Long result = redisUtil.executeScript(Constants.REDIS_SCRIPT, orderTokenKey, form.getOrderToken());
    ...
    public Long submit(OrderForm form) {
        ...
        String orderConfirmVOKey = String.format(Constants.ORDER_CONFIRM_REDIS_KEY, userInfoVO.getUserId());
        OrderConfirmVO orderConfirmVO = GsonUtil.json2Obj(redisUtil.getString(orderConfirmVOKey), OrderConfirmVO.class);
        executor.execute(() ->  redisUtil.deleteKey(orderConfirmVOKey));
        ...
        Map<Integer, SkuStock> stockMap = skuStockService.getSkuStockList(skuIdSet).stream().collect(Collectors.toMap(SkuStock::getSkuId, v -> v));
        /* 验证库存是否充足 */
        ...
        orderServiceProxy.insert(orderNo, userInfoVO.getUserId(), form.getCouponId(), orderItemList, orderShippingAddress, stockList, orderConfirmVO);
    ...
    public void insert(Long orderNo, Integer userId, Integer couponId, List<OrderItem> orderItemList,
        ...
        rabbitMQService.send(QueueEnum.ORDER_CLOSE_QUEUE, orderNo, ORDER_TIMEOUT);      // 发送延迟消息,15分钟订单未支付自动关闭
    }
    ...
    private OrderConfirmVO buildOrderConfirmVO(String orderToken, List<UserShippingAddressVO> userShippingAddressVOList,
        ...
        userCouponVOList = CollectionUtils.isEmpty(userCouponVOList) ? Collections.emptyList() : userCouponVOList.stream()
                .filter(e -> amount.compareTo(e.getCoupon().getCondition()) > -1)
                .filter(e -> new Date().before(e.getCoupon().getEndDate()))
                .collect(Collectors.toList());                  // 筛选有效优惠券
        OrderConfirmVO orderConfirmVO = new OrderConfirmVO();
        orderConfirmVO.setShippingAddressList(userShippingAddressVOList);
        orderConfirmVO.setCouponList(userCouponVOList);
